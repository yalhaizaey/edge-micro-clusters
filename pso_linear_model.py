...
def prMatrix(x):
    for row in x:
        ...
        print()
    print()
n=10 ## Thenumber of tasks to be allocated
...
def nodes_cap(particle):
    ...
    node_cap = [n*0.2]*3 + [n*0.2]*4 + [n*0.6]*1 ## 3nodes(RPi2), 4nodes(RPi3), 1node(RPi4)
...
# def fitness_fun(particle):
#     fitnessVal = 0.0
#     m = 5 ## line slop   (based on RPi4 observations)
#     b = 5 ## y-intercept (based on RPi4 observations)
#     particle = [int (i) for i in particle]
#     counter=collections.Counter(particle)
#     ''' call nodes_cap function to check node capacity constraints
#         if nodes constraints is False, FitnessVal is (inf)
...
#     else:
#         for i in (counter):
#             fitnessVal += (m * counter.get(i)) + b  ##(y=mx+b)
#     return fitnessVal
##------Function to return the devices with maximum exctime---------------------
def findmax(best_position):
    '''To find the node with maximum execution time for old fitness function'''
    best_position = [int (i) for i in best_position]
    print ("Best Allocation:", best_position) ##List 1 (allocation nodes)
    fitnessVal = 0.0
    excTime = []
